...
def IC(loss, changepoints, g_index, N, T, K, C=0, loss_fun='no_scale', Kl_fun='Nlog(NT)/T', h='1', C_K = 0):
    ...
    C_K : The coefficient in the penalty function Kl_fun. The default is 2. Note that I'm not sure how to set the default value. 
          ...
          If C_K=2 doesn't work for you, you may consider changing it to something else like O(sqrt(log(NT))).
    ...
    Ch = occurCount.dot((T - 1 - changepoints)[np.s_[indicesList]]) / (N * T) * C_K * h_in_IC(changepoints, T, h=h)
    ic = loss - Kl + Ch
    print("ic =", ic, ', ', K, '* l =', Kl, ', ', C, '* h =', Ch)
    return ic
