...
from simu.simulate_data_1d_flexible import *
from sklearn.tree import DecisionTreeRegressor
import simu.compute_test_statistics as stat_pvalue
...
print(data_path)
os.chdir(data_path)
file_name = 'seed_'+str(seed)+'newcp.dat'
...
# changepoint_list = [int(T_initial/2)  + int(0.2 * T_initial)- 1, int(T_initial/2) - int(0.2 * T_initial) - 1] # 如果这里为29， 那么第30个点是按照新的transition function计算的
if trans_setting == "smooth":
    changepoint_list = [int(T_initial/2)  + int(0.2 * T_initial)- 1+2, int(T_initial/2) - int(0.2 * T_initial) - 1+2] # 如果这里为29， 那么第30个点是按照新的transition function计算的
else:
    changepoint_list = [int(T_initial/2)  + int(0.2 * T_initial)- 1, int(T_initial/2) - int(0.2 * T_initial) - 1] # 如果这里为29， 那么第30个点是按照新的transition function计算的
