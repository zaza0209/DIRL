...
def setpath(trans_setting, init = "true clustering"):
    ...
    path_name = 'results/threshold_'+ threshold_type+'/sim_result_trans' + trans_setting +'/N' + str(N) +'/T_' + str(T)+'_init_'+init+'_K'+ str(K)+\
...
sys.stdout = open("log_"+str(random_cp_index)+".txt", "w")
...
if init == "true_clustering":
    ...
    out = mean_detect.fit(States, Actions, example="cdist", seed = seed,
                          ...
                          max_iter=max_iter, is_cp_parallel = 0)
    print('finish time', datetime.now()-startTime)
