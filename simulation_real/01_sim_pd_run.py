'''
Simulate 2->3 clusterings 2-dimensional time series data
...
# signal = [[0, 0], [0, 0, 0]]
# coef =[[[0.1, 0.5, 0.5],[0.1, 0.25, -0.75]], 
#         [[0, 0, 0.5],[0.5, 0.1, 0.25], [0,-0.25,-0.25]]] # shift in mean
...
def plot_fun(dim = 0):
    ...
    plt.vlines([59,49,39], -4,3.5, linestyles='dashed', colors='r')
    plt.show()
    
    plt.figure()
    plt.plot(States[:int(N/6):int(N/12),:,dim].T, 'y', label="i="+str(0)+","+str(int(N/12)))
    plt.plot(States[(2*int(N/6)):int(N/2):int(N/12),:,dim].T, 'b--', label="i="+str((2*int(N/6)))+","+str((2*int(N/6))+int(N/12)))
    plt.vlines(59, -3,3, linestyles='dashed', colors='r')
    plt.title("Group 1 1 split")
    ...
    plt.figure()
    plt.plot(States[2*int(N/6):int(N/2):int(N/12),:,dim].T, 'y', label="i="+str((2*int(N/6)))+","+str((2*int(N/6))+int(N/12)))
    plt.plot(States[int(N/2):int(2*N/3):int(N/12),:,dim].T, 'b--', label="i="+str(int(N/2))+","+str(int(N/2)+int(N/12)))
    plt.vlines(49, -3,3, linestyles='dashed', colors='r')
    plt.title("Group 1 1 and Group 1 2 merge")
    handles, labels = plt.gca().get_legend_handles_labels()
    by_label = dict(zip(labels, handles))
    plt.legend(by_label.values(), by_label.keys())
    plt.show()
    ...
    plt.figure()
    plt.plot(States[int(N/2):int(2*N/3):int(N/12),:,dim].T, 'y', label="i="+str(int(N/2))+","+str(int(N/2)+int(N/12)))
    plt.plot(States[int(2*N/3):int(2*N/3)+int(N/12)*3:int(N/12),:,dim].T, 'b--', label="i="+str(int(2*N/3))+","+str(int(2*N/3)+int(N/12)))
    plt.vlines(39, -3,3, linestyles='dashed', colors='r')
...
def setpath(trans_setting, K=None, init=None):
    ...
    # else:
    path_name = 'results/2x3/'+init+'/K'+str(K)+'/sim_result_trans' + trans_setting + '_reward' + reward_setting + '_gamma' + re.sub("\\.", "", str(gamma)) +\
                '_N' + str(N) + '_T'+ str(T)+'clusterws'+str(clustering_warm_start) +'coef'+   re.sub("\\.", "", re.sub("\\]","", re.sub("\\[", "", re.sub("\\,", "", str(coef)))))+ 'signal' + str(signal) + 'cov' + str(cov)
...
for K in [3]:
    ...
    for seed in range(0, 5):
        ...
        out = mean_detect.fit(States, Actions, K=3, example="cdist",init="clustering", seed = seed, nthread=nthread, max_iter=5)
    ...
    print(sum(cluster_err_list)/len(cluster_err_list))
       #%%
        # file_name = "seed_"+str(seed)+".dat"
        # with open(file_name, "wb") as f:
        #     pickle.dump({'iter_num' : out[0], 'group':out[1],
        #                  'changepoint' :out[2], 
        #                  'runtime': runtimeone,
        #                  'loss':out[4],
        #                  "ic":out[5]}, f)
    ...
    tmp = {"changepoint_err":sum(changepoint_err_list)/len(changepoint_err_list),
                 ...
                 "var_pe":np.var(changepoint_err_list),
                 "var_ce":np.var(cluster_err_list),
                 "var_iter":np.var(iter_num),
                 # "var_time":np.var(runtime/np.timedelta64(1, 's'))
                 }
    if shift == "linear":
        result_condis[0] = tmp
    elif shift =="abrupt":
...
for seed in range(20):
    ...
    # startTime = datetime.now()
    # out = mean_detect.fit_tuneK(range(1,6),States, Actions, example="cdist", seed = seed, nthread=nthread, max_iter=5)
    out = mean_detect.fit(States, Actions, K=3, example="cdist", seed = seed, nthread=nthread, max_iter=5)
...
for init in init_list:
    ...
    for seed in range(M):
        ...
        out = mean_detect.fit(States, Actions, K=3, example="cdist", changepoints_init=changepoints_init,
...
for seed in range(5):
    ...
    out = mean_detect.fit_tuneK(K_list,States, Actions, init="clustering", example="cdist", 
                                seed = seed, nthread=nthread, max_iter=5, 
                                C=C,changepoints_init = None, g_index_init_list=g_index_init_list,
                                clustering_warm_start=0)
    # bestK_list[seed]= out.K
    # print("best K",out.K)
    # print("changepoints", out.models[1].changepoints)
...
K_list = None
changepoint_err_list_ora = [[None]*M for i in range(2)]
cluster_err_list_ora = [[None]*M for i in range(2)]
for init in init_list:
    print('init',init)
    for seed in range(M):
        print("======= seed",seed, "==========")
        ...
        out3 = mean_detect.fit(States, Actions, K=3, example="cdist", seed = seed, 
                                ...
                                changepoints_init = changepoints_init,
                                clustering_warm_start=0,
    ...
    # print(np.mean(changepoint_err_list[10:20]))
    # print(np.mean(cluster_err_list[10:20]))
# print(np.mean(changepoint_err_mat, axis=1))
# print(np.mean(cluster_err_mat, axis=1))
# print(np.mean(bestK_list))
for i in range(len(init_list)):
    print("==== init", init_list[i], "=====")
    print(np.mean(changepoint_err_list_ora[i]))
    print(np.mean(cluster_err_list_ora[i]))
...
dat_all_cp = pd.DataFrame({"init":init_list,
              ...
              "cluster_err":np.mean(cluster_err_mat_cp, axis=0),
...
for init in init_list:
    if init == "randomclustering":
        ...
        for seed in range(M):
            ...
            # ic_list[K_list.index(K)] = uti.IC(t['loss'], t['changepoint'], 
            #                               t['group'], N, T, K, C)
            loss_mat_K[seed, 4] = t['loss']
            Kl_mat[seed, 4] =  K*np.log(np.sum(T-1 -t['changepoint']))
            Ck, indicesList, occurCount = np.unique(t['group'], return_index = True,return_counts=True)
            c_mat[seed, 4] = occurCount.dot((T-1 -t['changepoint'])[np.s_[indicesList]])/(N*T)*np.log(np.sum(T-1 -t['changepoint']))
            changepoint_err_mat_K[seed,4],cluster_err_mat_K[seed,4] = evaluate(changepoints_true.squeeze(), t['group'].squeeze(),
                                                    t['changepoint'].squeeze(), N, T)
            iter_mat_K[seed,4] = t['iter_num']
            pkl_file.close()
...
## all performance
dat_all_K = pd.DataFrame({"K":[2,3,4,5,3],
                      "changepoint_err":np.mean(changepoint_err_mat_K, axis=0),
                      "cluster_err":np.mean(cluster_err_mat_K, axis=0),
...
dat_all_K = dat_all_K.iloc[[4]+list(range(4))]
os.chdir("C:/Users/test/Dropbox/tml/IHS/simu/simu")
# file_name="results/2x3/res_K_list"+"init"+init+".xlsx"
# dat_K.to_excel(file_name)  
# #%% analyse result 3 -- K_list all K's performance
# K_list = range(2, 6)
# loss_mat = np.zeros([len(K_list),M])
# init = "randomclustering"
# changepoint_err_mat = np.zeros([len(K_list), M])
# cluster_err_mat = np.zeros([len(K_list), M])
# iter_mat = np.zeros([len(K_list), M])
# for seed in range(M):
#     for K in K_list:
#         setpath(trans_setting, K = K, init=init)
#         file_name = "seed_"+str(seed)+".dat"
#         pkl_file = open(file_name, 'rb')
#         t = pickle.load(pkl_file)
#         changepoint_err_mat[K_list.index(K), seed], cluster_err_mat[K_list.index(K), seed] = evaluate(changepoints_true.squeeze(), t['group'].squeeze(),
#                                                 t['changepoint'].squeeze(), N, T)
#         loss_mat[K_list.index(K), seed] = t['loss']
#         iter_mat[K_list.index(K), seed] = t['iter_num']
#         pkl_file.close()
# loss_K = np.mean(loss_mat, axis = 1)
# changepoint_err_K = np.mean(changepoint_err_mat, axis=1)
# cluster_err_K = np.mean(cluster_err_mat, axis=1)
# iter_K = np.mean(iter_mat, axis=1)

# dat_K = np.vstack([K_list, changepoint_err_K, cluster_err_K ,
#                    # loss_K, 
#                                 iter_K, 
#                                 np.std(changepoint_err_mat, axis=1)/np.sqrt(M),
#                                 np.std(cluster_err_mat, axis=1)/np.sqrt(M),
#                                 # np.std(loss_mat, axis=1)/np.sqrt(M),
#                                 np.std(iter_mat, axis=1)/np.sqrt(M)]).T
# dat_K = pd.DataFrame(dat_K, columns=['K','changepoint_err', 'cluster_err',
#                                      'iter_num','cp_var','cluster_var',
#                                      'iter_num_var'])
# dat_K = round(dat_K, 3)
# for i in range(1,4):
#     dat_K.iloc[:,i] = [str(a)+"("+str(b)+")" for a, b in zip(dat_K.iloc[:,i], dat_K.iloc[:,i+3])]

# cols = dat_K.columns.tolist()
# dat_K = dat_K[cols[:4]]
# os.chdir("C:/Users/test/Dropbox/tml/IHS/simu/simu")
# file_name="results/2x3/res_K_list"+"init"+init+".xlsx"
# # dat_K.to_excel(file_name)  
    ...
    if init == "randomclustering":
        ...
        for seed in range(M):
            for K in K_list:
                ...
                # ic_list[K_list.index(K)] = uti.IC(t['loss'], t['changepoint'], 
                #                               t['group'], N, T, K, C)
                loss_mat[init_list.index(init)][seed, K_list.index(K)] = t['loss']
        ...
        for seed in range(M):
            ...
            # ic_list[K_list.index(K)] = uti.IC(t['loss'], t['changepoint'], 
            #                               t['group'], N, T, K, C)
            loss_mat[init_list.index(init)][seed, 0] = t['loss']
