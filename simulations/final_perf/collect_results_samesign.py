...
file_name_index = [#'all','(14)','(23)','(25)', '(16)',
'(7)']
...
def run(calculate_err, calculate_ic,is_save_all, inits, N=N):
    if is_save_all:
        ...
        print('res_list',res_list)
            # res_list[j]['Method'] = np.concatenate([([i]*M) for i in type_all], axis=0)
            # dis_list[j] = pd.DataFrame(dis_list[j])
            # av_list[j].coloums = type_all
            # dis_list[j].coloums = type_all
        if is_save:
            os.chdir(path_original+ "/results/")
            # for setting in trans_setting:
            file_name="samesign_all_"+str(today)+'N' + str(N) + "_1d.csv"          
    ...
    if calculate_err :
            ...
            for N in Ns:
                ...
                for init in inits:
                    ...
                    for seed in seeds:
                        os.chdir(path_original+ "/results/")
                        print('seed',seed)
                        path_name = './threshold_'+ threshold_type+'/sim_result_trans' + setting +'/effect_size_samesign_sys0.25'+'/N' + str(N) +'/T_' + str(T)+'_init_'+init+\
                            '_1d/cov'+str(cov) + '/seed'+str(seed)
                        print('path_name',path_name)
                        if os.path.exists(path_name):
        ...
        if is_save:
            ...
            file_name="samesign_error"+str(today)+"_trans"+str(trans_setting) +'N' + str(Ns) + "_1d.xlsx"
    ...
    if calculate_ic:
            ...
            for N in Ns:
                ...
                for seed in range(M):
                    ...
                    for init in inits:
                        print('----- init', init)
                        os.chdir(path_original+ "/results/")
                        path_name = './threshold_'+ threshold_type+'/sim_result_trans' + setting +'/effect_size_samesign_sys0.4'+'/N' + str(N) +'/T_' + str(T)+'_init_'+init+\
                            '_1d/cov'+str(cov) + '/seed'+str(seed)
                        print(path_name)
                        if os.path.exists(path_name):
                            os.chdir(path_name)
                            ...
                            if re.search("kmeans", init):
                                ...
                                if os.path.exists(file_name):
                                    ...
                                    print(' loss', t['loss'], ', IC', IC_tmp,', cp', t['cp_err'], ', ARI', t['ARI'])
                                    if IC_max is None:
                                        print('MAX!!!')
                                        IC_max = IC_tmp
                                        best_model = init
                                ...
                                if os.path.exists(file_name):
                                    ...
                                    tmp = [setting, seed, init, t['cp_err'], t['ARI']]
                                    res_list.append(tmp)
                                    IC_tmp = ut.IC(t['loss'], t['changepoints'], t['clustering'], N, T,K=2, C=C)
                                    print(' loss', t['loss'], ', IC', IC_tmp,', cp', t['cp_err'], ', ARI', t['ARI'])
                                    if IC_max is None:
                                        print('MAX!!!')
                                        IC_max = IC_tmp
        ...
        if is_save:
            ...
            file_name="samesign_ic"+str(today)+"_trans"+str(trans_setting) +'N' + str(Ns)+file_name_index[init_list.index(inits)]+ "_1d.xlsx"
            with pd.ExcelWriter(file_name) as writer:
                for setting in trans_setting:
                    for N in Ns:
            ...
            # os.chdir(path_original+ "/results/")
            # # for setting in trans_setting:
            # file_name="icmodel_"+str(today)+'N' + str(N) + "_1d.csv"          
            # res_list.to_csv(file_name, index=False, header=['Setting','seed','init','cp_err', 'ARI'])
